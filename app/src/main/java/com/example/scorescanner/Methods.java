...
public class Methods extends AppCompatActivity {
    ...
    public static Bitmap imgSbdMade;
...
//        }
    ...
    private Bitmap getDataFromDB(Bitmap bitmap) {
        ...
        try {
            ...
            String made = getMade();
            String sbd = getSbd();
            Log.i(TAG, "getDataFromDB: === made = "+made);
            Log.i(TAG, "getDataFromDB: === sbd = "+sbd);
            double hediem = 0;
            String list_answer = "";
            if (made.contains("#")) {
            ...
            if (isExist.getCount() > 0) {
                ...
                valuediem.put("masv", sbd);
                valuediem.put("hinhanh", imguri);
                db.mydatabase.insert("diem",null,valuediem);
            }
...
//            Mat thresh = new Mat();
//            Imgproc.adaptiveThreshold(imgGray, thresh, 255,
//                    Imgproc.ADAPTIVE_THRESH_MEAN_C, Imgproc.THRESH_BINARY, 201, 3);
//            Utils.matToBitmap(thresh, imgSbdMade);
//            return imgSbdMade;
        } catch (Exception e) {
            e.printStackTrace();
            return bitmap;
